...
Three additional MCP tools that operate on courses you (or the agent) already generated. Same license tier as the generation tools (**Enterprise + AI add-on**) and they share the same daily quota — 5 calls per key per day, 50 per tenant per day. Each call burns one Gemini request.
